...
def get_game_state(player_id, game_id) -> GameResponse:
    ...
    gr.opponent_player_id = (
        g.player_2_id if g.player_1_id == player_id else g.player_1_id
    )
    return gr
...
def _check_clash_and_update_game(
    ...
    current_player_color_code = (
        "W" if current_player_color == PlayerColor.WHITE else "B"
    )
    ...
    if scoring_player_id:
        if (
            g.white_player_score == WINNING_SCORE
...
def _handle_piece_move(user_input: UserInput, g: Game) -> GameResponse:
    ...
    valid_coordinates = lambda coord: (
        len(coord) == 2 and 0 <= coord[0] <= 3 and 0 <= coord[1] <= 3
    )
    if not valid_coordinates(user_input.new_circle_position) or not valid_coordinates(
        user_input.new_triangle_position
    ):
        return GameResponse.from_game(g).with_error(
            "Invalid coordinates: "
            + str(user_input.new_circle_position)
            + " "
    ...
    last_move = (
        Move.objects.filter(game__exact=g)
        .filter(player_id__exact=user_input.player_id)
        .latest("move_timestamp")
    )
    if (
        last_move
        and datetime.datetime.now() - last_move.move_timestamp > MAX_TIME_PER_MOVE
    ):
        g.game_state = GameState.FINISHED.name
        g.winner_id = (
            g.black_player_id if color == PlayerColor.WHITE else g.white_player_id
        )
        g.save()
        return GameResponse.from_game(g)

...
class UserInput:
    ...
    def from_dict(in_dict: Dict[str, str]):
        ...
        user_input.new_triangle_position = in_dict.get("new_triangle_position",
                                                       None)
        user_input.new_circle_position = in_dict.get("new_circle_position",
                                                     None)
        return user_input
...
def create_game(player_id) -> GameResponse:
    ...
    g = Game.objects.create(player_1_id=player_id,
                            game_state=GameState.GAME_SETUP.name)
    g.save()
...
def join_game(player_id, game_id) -> GameResponse:
    ...
    if g.player_1_id == player_id:
        return GameResponse.from_game(g).with_error(
...
def get_game_state(player_id, game_id) -> GameResponse:
    ...
    gr.opponent_player_id = (g.player_2_id
                             if g.player_1_id == player_id else g.player_1_id)
    return gr
...
def _handle_color_choice(user_input: UserInput, g: Game) -> GameResponse:
    ...
    if user_input.player_id == g.player_1_id:
        if (g.player_1_color_choice != ""
                and g.player_1_color_choice != user_input.color_choice_shape):
            return GameResponse.from_game(g).with_error(
                "The color choice is already stored!")
        g.player_1_color_choice = user_input.color_choice_shape
        g.save()
    elif user_input.player_id == g.player_2_id:
        if (g.player_2_color_choice != ""
                and g.player_2_color_choice != user_input.color_choice_shape):
            return GameResponse.from_game(g).with_error(
                "The color choice is already stored!")
...
def _find_board_field(g: Game,
                      content: str) -> Tuple[Optional[int], Optional[int]]:
...
def _check_clash_and_update_game(
    g: Game,
    current_player_color: PlayerColor,
    new_triangle_position: Tuple[int, int],
    new_circle_position: Tuple[int, int],
) -> str:
    """Checks if there's a clash. Updates the game and returns the ID of scoring player (or an empty string)"""
    current_player_color_code = ("W" if current_player_color
                                 == PlayerColor.WHITE else "B")
    ...
    (other_tx,
     other_ty) = _find_board_field(g, "N" + other_player_color_code + "T")
    (other_cx,
     other_cy) = _find_board_field(g, "N" + other_player_color_code + "C")
    ...
    if scoring_player_id:
        if (g.white_player_score == WINNING_SCORE
                or g.black_player_score == WINNING_SCORE):
...
def _handle_piece_move(user_input: UserInput, g: Game) -> GameResponse:
    ...
    if not user_input.new_triangle_position or not user_input.new_circle_position:
        return GameResponse.from_game(g).with_error(
            "Missing new piece positions!")
    valid_coordinates = lambda coord: (len(coord) == 2 and 0 <= coord[0] <= 3
                                       and 0 <= coord[1] <= 3)
    if not valid_coordinates(
            user_input.new_circle_position) or not valid_coordinates(
                user_input.new_triangle_position):
        return GameResponse.from_game(g).with_error(
            "Invalid coordinates: " + str(user_input.new_circle_position) +
            " " + str(user_input.new_triangle_position))
    ...
    last_move = (Move.objects.filter(game__exact=g).filter(
        player_id__exact=user_input.player_id).latest("move_timestamp"))
    if (last_move and datetime.datetime.now() - last_move.move_timestamp >
            MAX_TIME_PER_MOVE):
        g.game_state = GameState.FINISHED.name
        g.winner_id = (g.black_player_id
                       if color == PlayerColor.WHITE else g.white_player_id)
        g.save()
        return GameResponse.from_game(g)
    ...
    if tx == cx and ty == cy:
        return GameResponse.from_game(g).with_error(
            "Invalid position. Same-color figures overlap.")
    if g.board[ty][tx] == "DF" or g.board[cy][cx] == "DF":
        return GameResponse.from_game(g).with_error(
            "Invalid position. Can't move to the destroyed field.")
    ...
    if last_move:
        ...
        if (abs(old_tx - tx) > 1 or abs(old_cx - cx) > 1
                or abs(old_ty - ty) > 1 or abs(old_cy - cy) > 1):
            return GameResponse.from_game(g).with_error(
                "Invalid position. New position is too far.")

    # Update the state & board
    scoring_player_id = ""
    if g.game_state in [
            GameState.INITIAL_PLACEMENT.name,
            GameState.GAME_RUNNING.name,
            GameState.CLASH.name,
    ]:
        if user_input.player_id == g.player_1_id:
    ...
    elif g.game_state in [
            GameState.WAITING_PLAYER_ONE_MOVE.name,
            GameState.WAITING_PLAYER_TWO_MOVE.name,
    ]:
    ...
    new_move = Move.objects.create(
        ...
        move_timestamp=datetime.datetime.now(),
        triangle_position=json.dumps([tx, ty]),
        circle_position=json.dumps([cx, cy]),
...
def handle_game_action(user_input: UserInput) -> GameResponse:
    ...
    try:
        ...
        return GameResponse(
            game_id=user_input.game_id).with_error("Invalid game ID")
    ...
    for s in GameState:
        ...
        return GameResponse(
            game_id=user_input.game_id).with_error("Invalid game state: " +
                                                   state)
    ...
    if state == GameState.GAME_RUNNING:
        ...
        return GameResponse(game_id=user_input.game_id).with_error(
            "Invalid game state for this action: " + state)
